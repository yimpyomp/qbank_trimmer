...
def generate_question_pdf(catalog, output_path=GENERATED_DIR / "selected_questions.pdf"):
    page_numbers = collect_blank_pages(catalog)

    return write_pages_to_pdf(COMBINED_QUESTIONS_PATH, page_numbers=page_numbers, output_path=output_path)


def generate_answer_pdf(catalog, output_path=GENERATED_DIR / "selected_answers.pdf"):
    page_numbers = collect_answer_pages(catalog)

    return write_pages_to_pdf(COMBINED_SOLUTIONS_PATH, page_numbers=page_numbers, output_path=output_path)
